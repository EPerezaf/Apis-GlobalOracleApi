...
/// <summary>
/// Controller para crear registros de sincronización de carga de proceso por dealer.
/// Ruta base: /api/v1/gm/dealer-sinc/confirmar-sinc-carga-proceso-dealer
/// </summary>
[ApiController]
[Route("api/v1/gm/dealer-sinc/confirmar-sinc-carga-proceso-dealer")]
[Produces("application/json")]
[Authorize]
public class CreateSincCargaProcesoDealerController : ControllerBase
{
    private readonly ISincCargaProcesoDealerService _sincCargaProcesoDealerService;
    private readonly ILogger<CreateSincCargaProcesoDealerController> _logger;

    public CreateSincCargaProcesoDealerController(
        ISincCargaProcesoDealerService sincCargaProcesoDealerService,
        ILogger<CreateSincCargaProcesoDealerController> logger)
    {
        _sincCargaProcesoDealerService = sincCargaProcesoDealerService;
        _logger = logger;
    }
    ...
    /// - Si no existe el registro de evento de carga, retorna error 400 Bad Request
    ...
    public async Task<IActionResult> Crear([FromBody] CrearSincCargaProcesoDealerDto dto)
    {
        ...
        _logger.LogInformation(
            "🔷 [CONTROLLER] Creando registro de sincronización. DealerBac: {DealerBac}, EventoCargaProcesoId: {EventoCargaProcesoId}, CorrelationId: {CorrelationId}",
            dealerBac, dto.EventoCargaProcesoId, correlationId);

        try
        {
            var resultado = await _sincCargaProcesoDealerService.CrearAsync(dto, dealerBac, usuarioAlta);

            stopwatch.Stop();
            _logger.LogInformation(
                "✅ [CONTROLLER] Registro creado exitosamente. DealerBac: {DealerBac}, SincCargaProcesoDealerId: {SincCargaProcesoDealerId}, Tiempo: {ElapsedMs}ms",
                dealerBac, resultado.SincCargaProcesoDealerId, stopwatch.ElapsedMilliseconds);
